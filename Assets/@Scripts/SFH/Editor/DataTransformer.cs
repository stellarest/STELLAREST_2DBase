...
namespace STELLAREST_SFH
{
	...
	private static void ParseExcelDataToJson<Loader, LoaderData>(string filename) where Loader : new() where LoaderData : new()
	{
		...
		File.WriteAllText($"{Application.dataPath}/@Resources/SFH/Data/JsonData/{filename}Data.json", jsonStr);
	...
	private static List<LoaderData> ParseExcelDataToList<LoaderData>(string filename) where LoaderData : new()
	{
		...
		string[] lines = File.ReadAllText($"{Application.dataPath}/@Resources/SFH/Data/ExcelData/{filename}Data.csv").Split("\n");
