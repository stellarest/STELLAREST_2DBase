...
namespace STELLAREST_SFH
{
    public class UI_Joystick : UI_Base
    {
        ...
        {
            ShowJoystick(true);
            GlowBackgroundFocus(Vector2.zero);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            GlowBackgroundFocus(_moveDir, moveDist, maxDist);

            Vector2 newPos = _touchPos + (_moveDir * moveDist);
            _cursor.transform.position = newPos;
        }

        private float GetViewportScale() => Screen.width / 1080f;
        ...
        // 거리 매개변수도 받아서, 일정 이상 넘으면 반짝거리게 하는게 나을지도
        private void GlowBackgroundFocus(Vector2 moveDir, float? currentDist = null, float? maxDist = null)
        {
