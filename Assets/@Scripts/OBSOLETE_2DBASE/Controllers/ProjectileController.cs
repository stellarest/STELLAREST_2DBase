...
namespace STELLAREST_2D
{
    public class ProjectileLaunchInfoEventArgs : System.EventArgs
    {
        public ProjectileLaunchInfoEventArgs(ProjectileLaunchInfoEventArgs arg)
        {
            ...
            this.LocalScale = arg.LocalScale;
                    ...
                    float targetScaleY, float duration, float movementSpeed, float rotationSpeed, int maxBounceCount, int maxPenetrationCount)
        {
            ...
            if (Mathf.Abs(targetScaleX) > Mathf.Abs(localScale.x) && Mathf.Abs(targetScaleY) > Mathf.Abs(localScale.y))
            {
                TargetScale = new Vector3(targetScaleX, targetScaleY, 1);
                this.IsOnTargetScale = true;
            }
            else if (Mathf.Abs(targetScaleX) > Mathf.Abs(localScale.x))
            {
                TargetScale = new Vector3(targetScaleX, localScale.y, 1f);
                this.IsOnTargetScale = true;
            }
            else if (Mathf.Abs(targetScaleY) > Mathf.Abs(localScale.y))
            {
    ...
    public class ProjectileController : SkillBase
    {
        ...
        {
            transform.localScale = new Vector3(this.Value.LocalScale.x, this.Value.LocalScale.y, 1f);
        ...
        private IEnumerator CoScaleInterpolation()
        {
            ...
            while (percent < 1f)
            {
                ...
                transform.localScale = Vector3.Lerp(this.Value.LocalScale, this.Value.TargetScale, percent);
                yield return null;
            }
        }
