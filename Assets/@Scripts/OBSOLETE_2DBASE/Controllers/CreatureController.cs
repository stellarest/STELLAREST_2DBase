...
namespace STELLAREST_2D
{
    ...
    {
        public FixedValue.TemplateID.Creatures CreatureType { get; set; } = FixedValue.TemplateID.Creatures.None;
        ...
        public virtual void OnDamaged(CreatureController attacker, SkillBase from)
        {
            ...
            else
            {
                ...
                if (from.Data.CrowdControlCount != 0)
        ...
        public void TryCrowdControl(SkillBase from)
        {
            ...
            CrowdControlType[] ccTypes = new CrowdControlType[ccCount];
            for (int i = 0; i < ccTypes.Length; ++i)
                ccTypes[i] = from.Data.CrowdControlTypes[i];
            ...
            for (int i = 0; i < ccCount; ++i)
            {
                ...
                {
                    float ccDuration = 0f;
                    if (from.Data.CrowdControlDurations.Length != 0)
                        ccDuration = from.Data.CrowdControlDurations[i];

                    float ccIntensity = 0f;
                    if (from.Data.CrowdControlIntensities.Length != 0)
                        ccIntensity = from.Data.CrowdControlIntensities[i];
