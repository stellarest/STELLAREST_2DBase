...
namespace STELLAREST_2D
{
            ...
            for (int i = 0; i < MemberCount; ++i)
            {
                if (Members[i].IsLearned == false)
                {
                    if (i == 0) // FIRST GRADE SKILL
                    {
                        ...
                        {
                            if (unlockSkill.Data.TemplateOrigin == Book.UniqueDefaultTemplate)
                            {
                    ...
                    else // REST OF SKILLS
                    {
                        ...
                        {
                            if (deactiveSkill.Data.TemplateOrigin == Book.UniqueDefaultTemplate)
                            {
    ...
    public class SkillBook : MonoBehaviour
    {
        ...
        public FixedValue.TemplateID.Skill UniqueDefaultTemplate { get; private set; } = FixedValue.TemplateID.Skill.None;
        ...
        public void LateInit()
        {
            foreach (var group in SkillGroupsDict)
            {
                ...
                for (int i = 0; i < group.Value.MemberCount; ++i)
                {
                    ...
                    if (skill.Data.Type == SkillType.Unique)
                    {
                        ...
                        {
                            case FixedValue.TemplateID.Skill.US_Paladin_Default:
                            case FixedValue.TemplateID.Skill.Knight_Unique_Mastery:
                            ...
                            case FixedValue.TemplateID.Skill.NinjaMastery:
                                if(skill.Data.Grade == InGameGrade.Elite)
                                    this.UniqueDefaultTemplate = skill.Data.TemplateOrigin;
                                break;

                            case FixedValue.TemplateID.Skill.US_Paladin_Elite:
                            case FixedValue.TemplateID.Skill.Knight_Unique_Elite:
                            case FixedValue.TemplateID.Skill.ForestGuardian_Unique_Elite:
                                {
                                    if (skill.Data.TemplateID == (int)FixedValue.TemplateID.Skill.US_Paladin_Elite)
                            ...
                            case FixedValue.TemplateID.Skill.US_Paladin_Ultimate:
                            case FixedValue.TemplateID.Skill.Knight_Unique_Ultimate:
                            case FixedValue.TemplateID.Skill.PhantomKnight_Unique_Ultimate:

                            case FixedValue.TemplateID.Skill.ArrowMaster_Unique_Ultimate:
                            case FixedValue.TemplateID.Skill.ElementalArcher_Unique_Ultimate:
                            case FixedValue.TemplateID.Skill.ForestGuardian_Unique_Ultimate:
            ...
            this.LevelUp(UniqueDefaultTemplate);
        }
        ...
        public void OnAddSkillCooldownRatioHandler(FixedValue.TemplateID.Skill skillTemplateID, float addRatio)
        {
            ...
            // //currentSkill.Data.Cooldown = FixedValue.Numeric.PALADIN_MINIMUM_ATTACK_COOLDOWN;
            // currentSkill.Data.Cooldown = PALADIN_ATTACK_COOLDOWN_TEST;
            // this.Owner.CreatureAnimController.SetAttackSpeed(PALADIN_ANIM_ATT_SPEED_TEST);
        ...
        public void SetSkillCooldown(FixedValue.TemplateID.Skill skillType, float value, bool reactiveImmediately = false)
        {
            SkillBase currentSkill = GetLastLearnedSkillMember(skillType);
            if (currentSkill == null)
                return;

            float prevCooldown = currentSkill.Data.Cooldown;
            // float cooldown = currentSkill.Data.Cooldown;
            // currentSkill.Data.Cooldown = cooldown - (cooldown * addRatio);
            currentSkill.Data.Cooldown = value;

            if (reactiveImmediately)
            {
                Deactivate(skillType);
                Activate(skillType);
            }

            Utils.Log($"Result Skill({currentSkill.Data.Name} Cooldown : {prevCooldown} -> {currentSkill.Data.Cooldown})");
        }
        ...
        public SkillBase GetCurrentMasterySkill()
            => SkillGroupsDict[(int)UniqueDefaultTemplate].GetLastLearnedSkillMember();
