...
namespace STELLAREST_2D
{
    public class  SpawningPool : MonoBehaviour
    {
        ...
        private float _spawnInterval = 999f; // 이것도 나중에 데이터 시트로 뺴야함. 무조건.

        private const int MAX_MONSTER_COUNT = 1; // 나중에 데이터 시트로 빼야함

        private Coroutine _coUpdateSpawningPool;
        public bool Stopped { get; set; } = false;
        private void Start() =>_coUpdateSpawningPool = StartCoroutine(UpdateSpawningPool());
        ...
        private void TrySpawn()
        {
            ...
            MonsterController mc = Managers.Object.Spawn<MonsterController>(randPos, (int)FixedValue.TemplateID.Creatures.Chicken, Define.ObjectType.Monster, true);
