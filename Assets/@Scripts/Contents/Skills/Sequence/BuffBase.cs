...
namespace STELLAREST_2D
{
    public abstract class BuffBase : SkillBase
    {
        ...
        protected Data.BuffSkillData _buffData = null;
        public Data.BuffSkillData BuffData => _buffData;

        protected ParticleSystem[] _particles = null;
        public bool IsBuffOn { get; protected set; } = false;
        protected Coroutine _coBuff = null;

        public void StartBuff(CreatureController target, SkillBase skill, Data.BuffSkillData buffSkillData)
        {
            _target = target;
            _skill = skill;
            _buffData = buffSkillData;
            if (buffSkillData.IsOnParent)
            {
